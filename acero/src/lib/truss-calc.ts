...
export function checkAngleTension(
  ...
  const limitState =
    phiPnYield <= phiPnRupt ? "Fluencia (φ·Fy·Ag)" : "Rotura (φ·Fu·Ae)";
...
export function designTrussMembers(
  ...
  const top = checkAngleCompression(
    topAngle,
    Fy,
    forces.chordLength * 1000,
    1.0,
    topForceAbs,
  );
  ...
  const diag = checkAngleCompression(
    diagAngle,
    Fy,
    forces.diagonalLength * 1000,
    1.0,
    forces.maxDiagonal,
  );
  ...
  const vert = checkAngleCompression(
    vertAngle,
    Fy,
    forces.verticalLength * 1000,
    1.0,
    forces.maxVertical,
  );
