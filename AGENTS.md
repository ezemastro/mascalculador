...
```powershell
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location 'C:\Users\marce\mascalculador\hormigon'; npm run dev:all"
```

`dev:all` corre en paralelo (con `concurrently`):
- **API + SQLite** (`npm run server`, puerto `5177`) — persiste en `hormigon/data/storage.db` (fuera de git).
- **Vite** (`npm run dev`, puerto `5174`) — proxya `/api` a la API.
...
> **Auth hormigon**: idéntico a viga-continua, cookie `hc_session`, API en puerto `5177` (dev). El administrador se define por nombre de usuario (`ADMIN_USERNAMES`, por defecto `marcmastro`; endpoint `/api/admin/users` + pantalla Admin + módulo Cabezales). No es "el primer usuario registrado": al arrancar y en cada login el flag `is_admin` se sincroniza con esa lista.
...
> **Recuperación de contraseña hormigon**: same backend; variables en `hormigon/.env` (ejemplo en `hormigon/.env.example`).
