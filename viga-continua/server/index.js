// viga-continua backend: autenticación (registro abierto) + storage por
// usuario en SQLite. El frontend usa un shim de localStorage que sincroniza
// con /api/storage, así toda la lógica existente (que escribe en localStorage)
...
function sendResetMail(to, resetLink) {
  const mail = {
    from: mailFrom,
    to,
    subject: "Recuperación de contraseña — Viga Continua",
    html: `
      <p>Hola,</p>
      <p>Recibimos una solicitud para recuperar tu contraseña de <strong>Viga Continua</strong>.</p>
...
if (fs.existsSync(DIST_DIR)) {
  app.use(express.static(DIST_DIR, { maxAge: "30d", immutable: true }));
...
app.listen(PORT, () => {
  console.log(`viga-continua server listening on :${PORT}`);
