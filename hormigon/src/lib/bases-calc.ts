...
export interface BaseResult {
  // Geometría
  Areq: number; Ap: number; Lx: number; Ly: number; h: number; d: number;
  kx: number; ky: number;
  // Cargas
  Pu: number; qu: number;
  // Centrada — flexión
  ...
  phiVc_beam_x: number; phiVc_beam_y: number;
  // Armadura
...
function step1_Dimensions(input: BaseInput): { Areq: number; Lx: number; Ly: number } {
  // Areq = (PD + PL) * factor / qa   (factor 1.10 = peso propio + tierra)
  // qa en kN/m² → Areq en m² → se pasa a cm² (×10000)
  const factor = input.includeSelfWeight === false ? 1.0 : 1.10;
...
function step5_Bending(
  qu: number, Lx: number, Ly: number, input: BaseInput,
...
function step7_EffectiveDepth(
  input: BaseInput, Lx: number, Ly: number, bx: number, by: number,
...
function step8_Punching(
  ...
  Y: number, alphaS: number,
): {
  Vu: number; phiVc: number; OK: boolean;
  b0: number; A0: number; beta: number; F1: number; F2: number; F: number;
} {
...
function step9_BeamShear(
  qu: number, Lx: number, Ly: number,
  input: BaseInput, bx: number, by: number, d: number, fc: number,
): {
  Vux: number; Vuy: number; phiVc_x: number; phiVc_y: number; OK: boolean;
  bwx: number; bwy: number;
} {
  const { kx, ky } = vuelos(input, Lx, Ly);
...
function step10_Steel(
  Mnx: number, Mny: number,
  bx: number, by: number, Lx: number, Ly: number, d: number, hProm: number,
...
function step13_Spacing(
  Asx: number, Asy: number, Lx: number, Ly: number, rebD: number, cover: number,
): {
  db: number; nb_x: number; nb_y: number;
...
function designCentrada(input: BaseInput): BaseResult {
  ...
  const Ap = Lx * Ly;
  st.push(`1. Área requerida A_base = (${f1(input.PD)}+${f1(input.PL)})·1.10 / ${f1(input.qa)} kN/m² = ${f1(dims.Areq / 10000)} m² = ${f1(dims.Areq)} cm²`);
  ...
  st.push(`5. Vuelos: kx = ${kxFull ? `${Lx}−${input.cx}` : `(${Lx}−${input.cx})/2`} = ${f1(kx)} cm, ky = ${kyFull ? `${Ly}−${input.cy}` : `(${Ly}−${input.cy})/2`} = ${f1(ky)} cm`);
  ...
  const { d: dPredim, method } = step7_EffectiveDepth(input, Lx, Ly, bx, by, Mnx, Mny, input.fc);
  const d_rig = Math.max(kx, ky) / 1.5;
  ...
  st.push(`   Predim rigidez: d ≥ máx(vuelo)/1.5 = máx(${f1(kx)}, ${f1(ky)})/1.5 = ${f1(d_rig)} cm`);
  st.push(`   Predim flexión: dx = √(6.5·${f1(Mnx)}/(${by}·${fmt(input.fc * 0.1, 2)})) = ${f1(dx_flex)} cm, dy = √(6.5·${f1(Mny)}/(${bx}·${fmt(input.fc * 0.1, 2)})) = ${f1(dy_flex)} cm`);
  ...
  if (input.h !== undefined && input.h > 0) {
    h = input.h;
    d = h - cover;
    st.push(`   h adoptado manual: h = ${h} cm → d = h − recubrimiento = ${h} − ${cover} = ${f1(d)} cm`);
  } else {
    d = dPredim;
    h = Math.max(d + cover, 30);
    st.push(`   h = d + recubrimiento = ${f1(d)} + ${cover} = ${f1(d + cover)} cm → adoptado ${h} cm (mín 30 cm)`);
  }
  ...
  // Paso 8 — punzonado
  const Y = input.type === "centrada" ? 1 : input.type === "esquina" ? 0.5 : 0.75;
  const alphaS = input.type === "centrada" ? 40 : input.type === "esquina" ? 20 : 30;
  const punch = step8_Punching(Pu, qu, input.cx, input.cy, d, input.fc, Y, alphaS);
  st.push(`8. Punzonado: b0 = 2·(${input.cx}+${f1(d)})+2·(${input.cy}+${f1(d)}) = ${f1(punch.b0)} cm`);
  st.push(`   β = ${f1(punch.beta)}, Y = ${Y}, αs = ${alphaS}, F1 = ${f1(punch.F1)}, F2 = ${f1(punch.F2)} → F = ${f1(punch.F)}`);
  st.push(`   Vu = ${f1(Pu)} − ${fmt(qu, 6)}·${f1(punch.A0)} = ${f1(punch.Vu)} kN`);
  st.push(`   φVc = 0.75·${Y}·${f1(punch.F)}·${f1(punch.b0)}·${f1(d)}·√${input.fc}/120 = ${f1(punch.phiVc)} kN`);
  ...
  st.push(`   bwx = (5·${bx}+3·${Lx})/8 = ${f1(shear.bwx)} cm, bwy = (5·${by}+3·${Ly})/8 = ${f1(shear.bwy)} cm`);
  st.push(`   Vux = ${fmt(qu, 6)}·${Ly}·max(${f1(kx)}−${f1(d)},0) = ${f1(shear.Vux)} kN`);
  st.push(`   Vuy = ${fmt(qu, 6)}·${Lx}·max(${f1(ky)}−${f1(d)},0) = ${f1(shear.Vuy)} kN`);
  st.push(`   φVc_x = 0.75·${f1(shear.bwy)}·${f1(d)}·√${input.fc}/60 = ${f1(shear.phiVc_x)} kN, φVc_y = 0.75·${f1(shear.bwx)}·${f1(d)}·√${input.fc}/60 = ${f1(shear.phiVc_y)} kN`);
  ...
  // Paso 10 — armadura
  const steel = step10_Steel(Mnx, Mny, bx, by, Lx, Ly, d, hProm, input.fc, input.fy, kamin);
  const fc_kNcm2 = input.fc * 0.1;
  st.push(`10. Flexión:`);
  st.push(`    mnx = ${f1(Mnx)}/(0.85·${by}·${f1(d)}²·${fmt(fc_kNcm2, 3)}) = ${f4(steel.mnx)}`);
  st.push(`    mny = ${f1(Mny)}/(0.85·${bx}·${f1(d)}²·${fmt(fc_kNcm2, 3)}) = ${f4(steel.mny)}`);
  st.push(`    kax = max(ka(mnx), ${f4(kamin)}) = ${f4(steel.kax)}`);
  st.push(`    kay = max(ka(mny), ${f4(kamin)}) = ${f4(steel.kay)}`);
  st.push(`    Asx = ${f4(steel.kax)}·0.85·${f1(d)}·${by}·${input.fc}/${input.fy} = ${f2(steel.Asx)} cm²`);
  st.push(`    Asy = ${f4(steel.kay)}·0.85·${f1(d)}·${bx}·${input.fc}/${input.fy} = ${f2(steel.Asy)} cm²`);
  st.push(`    AsMín = 0.0018·b·h_prom = 0.0018·${Math.max(Lx, Ly)}·${f1(hProm)} ≈ ${f2(steel.AsMin)} cm²`);

  // Paso 11 — talón
  st.push(`11. Talón (espesor del borde): kmin = mín(${f1(kx)}, ${f1(ky)}) = ${f1(kmin)} cm`);
  if (input.hTalon !== undefined) {
    st.push(`    h_talón adoptado manual = ${f1(hTalon)} cm (sugerido ${f1(hTalonSug)} cm)`);
  } else {
    st.push(`    h_talón = máx(25 cm, h − kmin) = máx(25, ${f1(h)} − ${f1(kmin)}) = ${f1(hTalon)} cm`);
  }
  ...
  st.push(`12. Armadura necesaria: Asx_nec = máx(${f2(steel.Asx)}, ${f2(steel.AsMin)}) = ${f2(Math.max(steel.Asx, steel.AsMin))} cm²`);
  st.push(`    Asy_nec = máx(${f2(steel.Asy)}, ${f2(steel.AsMin)}) = ${f2(Math.max(steel.Asy, steel.AsMin))} cm²`);
  st.push(`    (Ø, cantidad y separación se eligen y verifican en la sección Armadura.)`);

  st.push("");
  st.push(`=== RESUMEN: ${er.length === 0 ? "✓ DISEÑO COMPLETO" : "✗ ERRORES"} ===`);

  return {
    Areq: dims.Areq, Ap, Lx, Ly, h, d, kx, ky,
    Pu, qu,
    Mux, Muy, Mnx, Mny,
    Vu_punch: punch.Vu, phiVc_punch: punch.phiVc, punchOK: punch.OK,
    Vux: shear.Vux, Vuy: shear.Vuy, phiVc_beam: Math.min(shear.phiVc_x, shear.phiVc_y),
    phiVc_beam_x: shear.phiVc_x, phiVc_beam_y: shear.phiVc_y,
    ...
    eX: 0, eY: 0, MuX_volc: 0, MuY_volc: 0,
    Rux: 0, Ruy: 0, h_vigaX: 0, h_vigaY: 0,
    As_supX: 0, As_supY: 0, As_infX: 0, As_infY: 0,
    Tux: 0, Tuy: 0, As_tensorX: 0, As_tensorY: 0, h_tensorX: 0, h_tensorY: 0,
    tronco_N: 0, tronco_Mx: 0, tronco_My: 0, tronco_Vx: 0, tronco_Vy: 0,
    tensorPending: false,
    steps: st,
    warnings: wr,
    errors: er,
...
function designVigaFundacion(input: BaseInput): BaseResult {
  ...
  st.push("");
  st.push("La zapata se dimensiona a flexión/corte/punzonado como base centrada (presión uniforme).");
  st.push("La viga de fundación re-centra la resultante: qu = Pu/(Lx·Ly).");
  st.push("");

  // Paso V1 — excentricidad y momento volcador
  const { e, bViga: b_viga } = medGeom(input, Lx, Ly);
  const Mu = Pu * e;
  if (input.type === "medianera-x") {
  ...
  if (input.type === "medianera-x") {
    st.push(`V4. Viga: b = máx(cx,20) = ${b_viga} cm (adoptado)`);
  } else {
    st.push(`V4. Viga: b = máx(cy,20) = ${b_viga} cm (adoptado)`);
  }
  st.push(`    d = √(6.5·${f1(Mnv)}/(${b_viga}·${fmt(fc_kNcm2, 3)})) = ${f1(d_viga)} cm`);
  st.push(`    h_viga = d + recubrimiento = ${f1(d_viga)} + ${cover} = ${f1(h_viga)} cm`);
  ...
  st.push(`V5. mn_viga = ${f1(Mnv)}/(0.85·${b_viga}·${f1(d_viga)}²·${fmt(fc_kNcm2, 3)}) = ${f4(mn_med)}`);
  ...
  st.push(`V6. As_inf = máx(As_sup/3, 2Ø12) = máx(${f2(As_sup / 3)}, ${f2(2 * aBar(12))}) = ${f2(As_inf)} cm²`);
...
function designTensor(input: BaseInput): BaseResult {
  ...
  const As_tensor = tensorPending ? 0 : Tu / (0.90 * fy_kNcm2);
  const h_tensor = Math.max(Lx / 5, 20);
  if (tensorPending) {
    st.push(`T2. Falta ingresar la altura H del tensor al fondo de zapata — dimensionar en resultados.`);
  ...
  st.push(`T6. Tensor: sección sugerida ${Math.round(h_tensor)}×${Math.round(h_tensor)} cm`);
...
function designEsquina(input: BaseInput): BaseResult {
  ...
  const label = subType === "viga-de-equilibrio" ? "VIGA DE EQUILIBRIO" : "TENSOR";
  st.push(`=== BASE DE ESQUINA — ${label} — CIRSOC 201 ===`);
  st.push("");
  st.push("La zapata se dimensiona a flexión/corte/punzonado como base centrada (presión uniforme).");
  st.push("El sistema de equilibrio re-centra la resultante para que la presión sea uniforme qu = Pu/(Lx·Ly).");
  st.push("");

  // Paso E1 — excentricidades
  const eX = (Lx - cx) / 2;
  const eY = (Ly - cy) / 2;
  st.push(`E1. Excentricidades: eX = (Lx − cx)/2 = (${Lx} − ${cx})/2 = ${f1(eX)} cm`);
  st.push(`    eY = (Ly − cy)/2 = (${Ly} − ${cy})/2 = ${f1(eY)} cm`);

  // Paso E2 — momentos volcadores
  const MuX_volc = Pu * eX;
  const MuY_volc = Pu * eY;
  st.push(`E2. Momentos volcadores: MuX = Pu·eX = ${f1(Pu)}·${f1(eX)} = ${f1(MuX_volc)} kN·cm`);
  ...
  if (subType === "viga-de-equilibrio") {
    ...
    st.push(`E3. Reacciones: Rux = MuX / LcolX = ${f1(MuX_volc)} / ${input.LcolX} = ${f1(Rux)} kN`);
    st.push(`    Ruy = MuY / LcolY = ${f1(MuY_volc)} / ${input.LcolY} = ${f1(Ruy)} kN`);
    st.push("");
    ...
    st.push(`E4. Viga X: b = máx(cy,20) = ${bX} cm | MnvX = MuX / 0.90 = ${f1(MnvX)} kN·cm`);
    st.push(`    d = √(6.5·${f1(MnvX)}/(${bX}·${fmt(fc_kNcm2, 3)})) = ${f1(dX)} cm → h = ${f1(hX)} cm`);
    ...
    st.push(`E5. Viga Y: b = máx(cx,20) = ${bY} cm | MnvY = MuY / 0.90 = ${f1(MnvY)} kN·cm`);
    st.push(`    d = √(6.5·${f1(MnvY)}/(${bY}·${fmt(fc_kNcm2, 3)})) = ${f1(dY)} cm → h = ${f1(hY)} cm`);
    ...
    st.push(`E3. Tracciones: Tux = MuX / Hx = ${f1(MuX_volc)} / ${Hx} = ${f1(Tux)} kN`);
    ...
    As_tensorX = tensorPending ? 0 : Tux / (0.90 * fy_kNcm2);
    As_tensorY = tensorPending ? 0 : Tuy / (0.90 * fy_kNcm2);
    if (tensorPending) {
      st.push("E4. Falta ingresar las alturas Hx/Hy de los tensores al fondo de zapata — dimensionar en resultados.");
      er.push("Faltan las alturas de los tensores (Hx, Hy) para dimensionarlos.");
    } else {
      st.push(`E4. Rozamiento: PD·μ = ${f1(input.PD)}·${mu} = ${f1(Rf)} kN`);
      st.push(`    Tux = ${f1(Tux)} kN, Tuy = ${f1(Tuy)} kN → ${FrictionOK ? "✓ Rozamiento ≥ Tu" : "✗ Rozamiento < Tu — ADVERTENCIA"}`);
      if (!FrictionOK) {
        wr.push(
          "La fuerza de rozamiento es menor que la tracción en el tensor (en una o ambas direcciones). Aumentar PD o μ.",
        );
      }
      st.push(`E5. As_tensorX = ${f1(Tux)} / (0.90·${fmt(fy_kNcm2, 1)}) = ${f2(As_tensorX)} cm²`);
      st.push(`    As_tensorY = ${f1(Tuy)} / (0.90·${fmt(fy_kNcm2, 1)}) = ${f2(As_tensorY)} cm²`);
    }

    h_tensorX = Math.max(Lx / 5, 20);
    h_tensorY = Math.max(Ly / 5, 20);
    st.push(`E6. Bloques tensor: hX = máx(Lx/5,20) = ${Math.round(h_tensorX)} cm | hY = máx(Ly/5,20) = ${Math.round(h_tensorY)} cm`);
    ...
    tronco_Vy = Tuy;
    st.push("");
    st.push("E7. TRONCO DE COLUMNA (columna corta) — fuerzas de diseño para verificación posterior:");
    st.push(`    N = ${f1(tronco_N)} kN | Mx = ${f1(tronco_Mx)} kN·cm | My = ${f1(tronco_My)} kN·cm`);
    st.push(`    Vx = ${f1(tronco_Vx)} kN | Vy = ${f1(tronco_Vy)} kN`);
    st.push("    NOTA: el tronco debe verificarse a flexo-compresión biaxial + axial con estas fuerzas.");
    st.push("");
  } else {
...
export function designBase(input: BaseInput): BaseResult {
  ...
  if (norm.qa <= 0) {
    throw new Error("La tensión admisible del suelo (σadm) debe ser mayor que cero.");
  }
  ...
  if (norm.type === "esquina") {
    if (!norm.subType) {
      throw new Error("Para base de esquina, seleccione viga de equilibrio o tensor (subType).");
    }
